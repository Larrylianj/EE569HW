...
#define MAT {1, 2, 1, 2, 4, 2, 1, 2, 1}

using namespace std;

unsigned char WeightedSum(int mat[], int len, unsigned char buf[]);

int main(int argc, char *argv[])

{
    // for debug only
//    char in_file[] = "/Users/liangsiyang/Documents/USC-learning/EE-569/HW1/images/Lena_median.raw";
//    char out_file[] = "/Users/liangsiyang/Documents/USC-learning/EE-569/HW1/images/Lena_linear.raw";
//    argc = 4;
//    argv[1] = in_file;
//    argv[2] = out_file;
//    *argv[3] = '3';
                ...
                for (int i = x - WINDOW_SIZE/2; i < x + WINDOW_SIZE/2; i++) {
                    for (int j = y - WINDOW_SIZE/2; j < y + WINDOW_SIZE/2; j++) {
                        if (i >= 0 && i < Size1 && j >= 0 && j < Size2) {
                            ...
                            buffer[count] = Imagedata[x][y][color];
                        }
                        count++;
                    }
                }
                OutputImagedata[x][y][color] = WeightedSum(weighted_matrix, count, buffer);
...
unsigned char WeightedSum(int mat[], int len, unsigned char buf[])
{
    int total = 0.0, sum = 0.0;
    for (int i = 0; i < len; i++)
        total += mat[i];
    for (int i = 0; i < len; i++)
        sum += mat[i] * (int)buf[i];
    return (unsigned char)(sum/total);
}
